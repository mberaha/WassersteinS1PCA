...
class PCA(metaclass=ABCMeta):
    ...
    def _process_data(self, distribs, base_point):
        self.ndata = len(distribs)
        self.distribs = distribs
        if self.compute_spline:
            for d in self.distribs:
                d.compute_spline_expansion(self.spline_basis)
            
            base_point.compute_spline_expansion(self.spline_basis)

        self.coeff_mat = self.get_spline_mat(distribs)
        self.bary_coeff = base_point.quantile_coeffs
